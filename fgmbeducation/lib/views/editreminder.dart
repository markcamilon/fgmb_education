...
class EditReminderScreen extends StatefulWidget {
  final ReminderPalletteData reminderData;
  EditReminderScreen({required this.reminderData});
  @override
...
class _EditReminderScreenState extends State<EditReminderScreen> {
  ...
  late TimeOfDay formTime;
  var formattedFormTime;
  var formDate;
  ...
  void initState() {
    ...
    setState(() {
      _reminderMessage = widget.reminderData.message;
      formattedFormTime = widget.reminderData.time;
      checkBoxValue = widget.reminderData.type == "0" ? true : false;
      formTime = timeConvert(widget.reminderData.time);
    });
  ...
  Future<Null> _selectTime(BuildContext context) async {
    ...
    if (pickedTime != null) {
      ...
      setState(() {
        formTime = pickedTime;
        formattedFormTime = formTime.format(context);
  ...
  _deletePreviousReminder() {
    if (widget.reminderData.type == "0") {
      flutterLocalNotificationsPlugin.cancel(
        int.parse(
          widget.reminderData.notificationKey,
        ),
      );
    } else {
      var keys = jsonDecode(widget.reminderData.notificationKey);
      keys.forEach(
        (element) {
          flutterLocalNotificationsPlugin.cancel(
            int.parse(
  ...
  _addReminder() async {
    ...
    var response = await http.post(
      Uri.parse(APIRoutes.editReminder),
      body: {
        "api": apiKey,
        "id": widget.reminderData.id.toString(),
  ...
  _deleteReminder() async {
    ...
    var apiKey = prefs.getString("api");
    var response = await http.post(
      Uri.parse(APIRoutes.deleteReminder),
      body: {
        "api": apiKey,
        "reminderID": widget.reminderData.id.toString(),
      },
    );
    if (response.statusCode == 200) {
      var jsonResponse = json.decode(response.body);
              ...
              child: Form(
                ...
                child: Column(
                  children: [
                    ...
                    if (!checkBoxValue)
                      Text(
                        "Old Days: " + widget.reminderData.day,
                        style: TextStyle(
                          color: Colors.white,
                        ),
                      ),
                            ...
                            child: Center(
                              child: Row(
                                children: [
                                  ...
                                  Spacer(),
                                  Text(
                                    AppLocalizations.of(context)!.save_and_exit,
                                    style: TextStyle(
                                      color: Colors.white,
                                      fontSize: 20,
                                    ),
